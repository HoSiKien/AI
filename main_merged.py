...
from advanced_ai import smart_answer
from learning_ai import learning_ai
from learning_ai_advanced import process_learning  # dòng mới
from advanced_ai import call_gemini               # dòng mới (nếu cần dùng riêng)
...
async def scrape_tech():
    """Scrape tin công nghệ - CHỈ THÊM MỚI, KHÔNG XÓA"""
    try:
        news_list = scraper.scrape_tech_news()
        count = 0
        for news in news_list:
            # Kiểm tra trùng lặp trước khi thêm
            if not database.check_news_exists(news['title'], news['url']):
                ...
                count += 1
        return {"message": f"✅ Đã thêm {count} tin mới. Dữ liệu cũ được giữ nguyên."}
    except Exception as e:
        print(f"Lỗi: {e}")
...
async def rag_chat(question: RAGQuestion):
    results = rag.search(question.question, top_k=3)
    if not results:
        return RAGChatResponse(answer="❌ Chưa có dữ liệu. Hãy upload tài liệu về trường Tôn Đức Thắng trước.", sources=[])
    
    context = "\n\n---\n\n".join([chunk for chunk, _, _ in results])
    answer = smart_answer(question.question, context)
    sources = [f"[{r[2].get('doc_name', 'Unknown')}]: {r[0][:150]}..." for r in results]
...
async def learning_process(request: LearningRequest):
    """Xử lý toàn bộ 3 module SỬ DỤNG GEMINI (nâng cao)"""
    try:
        print(f"📚 Nhận yêu cầu học tập (bản nâng cao): {request.title[:50]}...")
        
        # Gọi hàm AI nâng cao để xử lý
        result = process_learning(request.content, request.title)
        
        # Chuyển đổi kết quả về đúng format mà frontend mong đợi
        # Frontend cần: summary, suggestions, questions
        # Ta sẽ map như sau:
        formatted_result = {
            "summary": {
                "title": request.title,
                "difficulty": "Trung bình",  # Có thể tự động đánh giá sau
                "estimated_time": "1-2 giờ",
                "summary": result.get("summary", ""),
                "key_points": result.get("explain", "").split('.')[:3],  # tạm lấy 3 ý từ explain
                "explanations": {"Khái niệm": result.get("explain", "")}
            },
            "suggestions": {
                "main_topic": "Công nghệ thông tin",
                "estimated_time": "1-2 giờ",
                "skills_to_learn": ["Tư duy logic", "Lập trình", "Đọc hiểu tài liệu"],
                "resources": [{"name": "Google Scholar", "type": "Tìm kiếm", "url": "https://scholar.google.com"}],
                "roadmap": [{"week": "Tuần 1-2", "focus": "Nắm vững khái niệm", "activities": "Đọc lại bài viết và ghi chú"}]
            },
            "questions": []
        }
        ...
        for idx, q in enumerate(questions):
            formatted_result["questions"].append({
                "id": idx+1,
                "type": "Hiểu",
                "question": q,
                "sample_answer": "Tham khảo trong bài viết",
                "difficulty": "Trung bình"
            })
        
        print("✅ Xử lý thành công (Gemini)")
        return formatted_result
        
    except Exception as e:
        print(f"❌ Lỗi learning_process: {str(e)}")
        # Fallback về learning_ai cũ nếu lỗi
        try:
            print("⚠️ Fallback về learning_ai cũ...")
